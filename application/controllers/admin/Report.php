...
class Report extends CI_Controller {
	...
	public function index()
	{
		...
		foreach ($result as $key => $value) {
			...
			$value->lunch_customer = $this->db->query("SELECT SUM(total_customer) as total_customer
												FROM billings 
												WHERE YEAR(date) ='{$value->year}'
												AND MONTH(date) = '{$value->month}'
												AND  start between '00:00' AND '16:00'")->row()->total_customer;
			$value->dinner_customer = $this->db->query("SELECT SUM(total_customer) as total_customer
												FROM billings 
												WHERE YEAR(date) ='{$value->year}'
												AND MONTH(date) = '{$value->month}'
												AND  start between '16:01' AND '23:59'")->row()->total_customer;
			$value->total_customer = $this->db->query("SELECT SUM(total_customer) as total_customer
												FROM billings 
												WHERE YEAR(date) ='{$value->year}'
